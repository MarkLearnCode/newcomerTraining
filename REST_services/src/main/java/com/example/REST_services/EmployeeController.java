...
//  }
  ...
  CollectionModel<EntityModel<Employee>> all() {
      List<EntityModel<Employee>> employees = repository.findAll().stream()
              .map(employee -> EntityModel.of(employee,
                      WebMvcLinkBuilder.linkTo(EmployeeController.class).slash(employee.getId()).withSelfRel(),
                      WebMvcLinkBuilder.linkTo(EmployeeController.class).withRel("employees")))
              .collect(Collectors.toList());

      return CollectionModel.of(employees, WebMvcLinkBuilder.linkTo(EmployeeController.class).withSelfRel());
  }
...
//  }
  ...
  EntityModel<Employee> one(@PathVariable Long id) {
    ...
    Link linkToAll = WebMvcLinkBuilder.linkTo(EmployeeController.class).withRel("employees");
    model.add(linkToAll);
